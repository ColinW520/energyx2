class DashboardController < ApplicationController
  def show
    @s_data = Submission.where(submissions: {created_at: 1.month.ago..Time.now}).joins(:participant).group("participants.name").count.sort_by {|k,v| v}.reverse
    ...
    @p_data = Participant.order(created_at: :desc).limit(10)
  end
end
